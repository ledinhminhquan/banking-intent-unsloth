...
(\texttt{Banking77\_Unsloth\_Colab.ipynb}) that walks through six stages, in
this exact order:
...
\begin{enumerate}[noitemsep]
    ...
    \item If that fails, load from the auto-generated parquet branch using
          the option \texttt{revision="refs/convert/parquet"}.
...
\begin{itemize}[noitemsep]
    ...
    \item \texttt{balance\_not\_updated\_after\_bank\_transfer}
...
For example, \texttt{card\_payment\_not\_recognised} has perfect recall but
low precision -- the model sometimes \textit{over-predicts} it for messages
that actually belong to \texttt{direct\_debit\_payment\_not\_recognised} or
\texttt{cash\_withdrawal\_not\_recognised}, which share most of their
vocabulary (\textit{``I do not recognise this payment''}). This is a known
failure mode in BANKING77 and is exactly the kind of error a larger training
set or a soft-label smoothing technique would help fix.
