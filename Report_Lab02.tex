...
%
% Recommended compiler on Overleaf: pdfLaTeX (works fine, no Unicode font needed)
% ==============================================================================
\documentclass[12pt,a4paper]{article}

% ---- Encoding (English-only report; Vietnamese names written in ASCII) ----
...
% ---- Hyperlinks: xurl makes long URLs wrap at any character ----
\usepackage{xurl}
\usepackage{hyperref}
\hypersetup{colorlinks=true, linkcolor=blue, citecolor=blue, urlcolor=blue,
            breaklinks=true}

% ---- Better typesetting + flexible line breaking for long monospace strings ----
...
% Encourage looser line-breaking globally so long monospace tokens do not
...
\begin{titlepage}
    ...
    {\large \textbf{Lecturer:} Dr. Nguyen Hong Buu Long}\\[1.5cm]
...
\begin{itemize}[noitemsep]
    \item \textbf{GitHub Repository (public):}\\ \repolink{}
    \item \textbf{Google Colab Notebook (Drive):}\\ \notebooklink{}
    \item \textbf{Demo Video (Drive, public):}\\ \videolink{}
\end{itemize}
...
The whole project is orchestrated by a single ``autopilot'' Colab notebook
(file name:
\texttt{Banking77\_\allowbreak Unsloth\_\allowbreak Colab.ipynb}).
It walks through six stages, in this exact order:

\begin{lstlisting}[language={},caption={High-level pipeline (notebook stages and what each one produces).}]
[Step 0] Install deps + verify GPU + set HF / unsloth cache dirs
              v
[Step 1] Clone repo from GitHub + write configs/ + render README.md
              v
[Step 2] %%writefile the three scripts
         (preprocess_data.py, train.py, inference.py)
              v
[Step 3] preprocess_data.py
              -> sample_data/{train,test}.csv + label_map.json
              v
[Step 4] train.py
              -> saved_model/ (LoRA adapter)
              -> evaluation_results.json
              v
[Step 5] inference.py demos + full evaluation
              -> 87.92% test accuracy
              v
[Step 6] Push everything (clean) to GitHub
              -> https://github.com/ledinhminhquan/banking-intent-unsloth
\end{lstlisting}

This means a grader can fully reproduce the project by simply opening the
committed notebook on Colab, attaching a GPU runtime, and clicking
\textit{Runtime $\rightarrow$ Run all}. No manual file editing is required
beyond pasting two Drive links at the top.
...
Recent releases of the \texttt{datasets} library (version 3 and above) no
longer execute arbitrary dataset loading scripts. Because the BANKING77 repo
originally shipped with a Python loading script, a naive
...
\begin{enumerate}[noitemsep]
    ...
    \item If that fails, switch to the auto-generated parquet branch by
          passing the argument
          \texttt{revision="refs/\allowbreak convert/\allowbreak parquet"}.
...
\subsubsection*{Best-Performing Classes (perfect F1 = 1.00)}

The model achieved a perfect classification (precision = recall = F1 = 1.00 with
support = 10) on the following intent classes:

\begin{itemize}[noitemsep]
    \item \texttt{age\_limit}
    \item \texttt{atm\_support}
    \item \texttt{balance\_not\_updated\_\allowbreak after\_bank\_transfer}
    \item \texttt{cancel\_transfer}
    \item \texttt{card\_swallowed}
    \item \texttt{edit\_personal\_details}
    \item \texttt{lost\_or\_stolen\_phone}
    \item \texttt{pending\_cash\_withdrawal}
    \item \texttt{pin\_blocked}
    \item \texttt{terminate\_account}
\end{itemize}

These are intents whose surface vocabulary is unambiguous (e.g.,
\textit{``what is the age limit''}, \textit{``my card got stuck in the
ATM''}), so even with 40 training samples per class the model learned them
trivially.

\subsubsection*{Hardest Classes (F1 \texttt{<} 0.70)}

The lowest-scoring classes were dominated by semantic overlap with sibling
intents:

\begin{table}[H]
    \centering
    \caption{Bottom classes by F1-score, with the most likely confusion
    partner in BANKING77 noted.}
    \label{tab:hard-classes}
    \begin{tabular}{lccc}
        \toprule
        \textbf{Class}                                & \textbf{P}  & \textbf{R}  & \textbf{F1}  \\
        \midrule
        \texttt{beneficiary\_not\_allowed}            & 0.71        & 0.50        & 0.59         \\
        \texttt{direct\_debit\_payment\_not\_recognised} & 1.00     & 0.50        & 0.67         \\
        \texttt{card\_payment\_not\_recognised}       & 0.50        & 1.00        & 0.67         \\
        \bottomrule
    \end{tabular}
\end{table}

For example, \texttt{card\_payment\_\allowbreak not\_recognised} has perfect
recall but low precision -- the model sometimes \textit{over-predicts} it for
messages that actually belong to a sibling class such as
\texttt{direct\_debit\_\allowbreak payment\_not\_recognised} or
\texttt{cash\_withdrawal\_\allowbreak not\_recognised}, which share most of
their vocabulary (\textit{``I do not recognise this payment''}). This is a
known failure mode in BANKING77 and is exactly the kind of error a larger
training set or a soft-label smoothing technique would help fix.
...
\begin{table}[H]
    ...
    All examples below were predicted correctly.}
    \label{tab:examples}
...
\begin{itemize}[noitemsep]
    \item \textbf{Hugging Face \texttt{datasets} v3+ removed support for
          ...
          fallback (default, parquet branch, direct parquet URLs).
...
The entire project is fully reproducible from the public GitHub repository.

\subsection{Option A -- One-Click on Colab (recommended)}

\begin{enumerate}[noitemsep]
    \item Open \texttt{Banking77\_Unsloth\_Colab.ipynb} from the repository
          (or from the Drive notebook backup).
    \item \texttt{Runtime $\rightarrow$ Change runtime type $\rightarrow$ GPU}
          (H100 / A100 / T4 all work).
    \item Add two Colab Secrets: \texttt{GITHUB\_PAT} (write scope) and
          \texttt{HF\_TOKEN} (optional, helps avoid HuggingFace rate limits).
    \item Click \texttt{Runtime $\rightarrow$ Run all}. Total wall-clock time
          on H100: about 6 minutes (1 min install, 4 min training, 1 min
          evaluation + push).
\end{enumerate}

\subsection{Option B -- Local CLI}

\begin{lstlisting}[language=bash,caption={Local reproduction commands (Linux / WSL).}]
# 1. Clone
...
# 5. Standalone inference (single message)
python scripts/inference.py \
    --config configs/inference.yaml \
    --message "I need to change my PIN number"
...
\begin{itemize}[noitemsep]
    \item Data sampling: \texttt{seed=42} (in \texttt{preprocess\_data.py}).
    \item Train/eval shuffling: same seed.
    \item Trainer: \texttt{seed=42}, \texttt{lora.random\_state=3407}.
    \item Inference: greedy decoding (\texttt{do\_sample=false},
          \texttt{temperature=0}).
\end{itemize}
However, exact reproduction of the \textbf{87.92\%} headline number can vary
slightly run-to-run because Unsloth, BitsAndBytes, and CUDA kernels introduce
small non-determinism in 4-bit quantization. We have observed a stable range
of \textbf{87--89\%} test accuracy across multiple H100 runs with the same
hyperparameters, which is the expected variance for this setup.
...
Casanueva, I., Temcinas, T., Gerber, D., Fitzpatrick, M., \& Vulic, I. (2020).
